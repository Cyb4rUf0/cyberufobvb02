...
def wait_for_price():
        ...
        if threshold_check > CHANGE_IN_PRICE:
            ...
            if datetime.now() >= volatility_cooloff[coin] + timedelta(minutes=TIME_DIFFERENCE):
                ...
                if len(coins_bought) + len(volatile_coins) < TRADE_SLOTS or TRADE_SLOTS == 0:
                    ...
                    print(f'{txcolors.WARNING}{coin} Has Gained {volatile_coins[coin]}% Within the last [{TIME_DIFFERENCE}] Minutes: Purchasing ${TRADE_TOTAL} {PAIR_WITH} of <{coin}>{txcolors.DEFAULT}')

                else:
                    print(f'{txcolors.WARNING}{coin} Has Gained {round(threshold_check, 3)}% Within the last [{TIME_DIFFERENCE}] Minutes: <IGNORED> Using All Trade Slots{txcolors.DEFAULT}')
            #else:
    ...
    for excoin in externals:
        if excoin not in volatile_coins and excoin not in coins_bought and \
            ...
            exnumber +=1
            print(f"{txcolors.WARNING}External Signal Received On: <{excoin}> | Purchasing ${TRADE_TOTAL} {PAIR_WITH} Value For <{excoin}>{txcolors.DEFAULT}")
...
def balance_report(last_price):
    ...
    print(f'')
    print(f'{txcolors.BLUE}                  |°*°*°*° HORUS ALGORITHMIC °*°*°*°|{txcolors.DEFAULT}')
    print(f'{txcolors.BORDER}+---------------------------------------------------------------------------+{txcolors.DEFAULT}')
    ...
    print(f'Realized PnL    : {txcolors.SELL_PROFIT if session_profit_incfees_perc > 0. else txcolors.SELL_LOSS}{session_profit_incfees_perc:.4f}% Est:${session_profit_incfees_total:.4f} {PAIR_WITH}{txcolors.DEFAULT}')
    print(f'Unrealized PnL  : {txcolors.SELL_PROFIT if unrealised_session_profit_incfees_perc > 0. else txcolors.SELL_LOSS}{unrealised_session_profit_incfees_perc:.4f}% Est:${unrealised_session_profit_incfees_total:.4f} {PAIR_WITH}{txcolors.DEFAULT}')
    print(f'   Total PnL    : {txcolors.SELL_PROFIT if (session_profit_incfees_perc + unrealised_session_profit_incfees_perc) > 0. else txcolors.SELL_LOSS}{session_profit_incfees_perc + unrealised_session_profit_incfees_perc:.4f}% Est:${session_profit_incfees_total+unrealised_session_profit_incfees_total:.4f} {PAIR_WITH}{txcolors.DEFAULT}')
    print(f'{txcolors.BORDER}+---------------------------------------------------------------------------+{txcolors.DEFAULT}')
    ...
    print(f'Cumulative PnL  : {txcolors.SELL_PROFIT if historic_profit_incfees_perc > 0. else txcolors.SELL_LOSS}{historic_profit_incfees_perc:.4f}% Est:${historic_profit_incfees_total:.4f} {PAIR_WITH}{txcolors.DEFAULT}')
    print(f'Completed Trades: {trade_wins+trade_losses} (Wins:{trade_wins} Losses:{trade_losses})')
    print(f'Win/Loss Ratio  : {float(WIN_LOSS_PERCENT):g}%')
    print(f'{txcolors.BORDER}+---------------------------------------------------------------------------+{txcolors.DEFAULT}')
    print(f'{txcolors.BLUE}                 |*-*-*-*-* CYB4RUF0 SYSTEMS *-*-*-*-*|{txcolors.DEFAULT}')
    print(f'')
    
    #msg1 = str(bot_started_datetime) + " | " + str(datetime.now() - bot_started_datetime)
    msg1 = "#" + str(datetime.now()).split('.')[0] + "# "
    msg2 = " | CCB: " + str(len(coins_bought)) + "/" + str(TRADE_SLOTS) + " | PBOT: " + str(bot_paused)
    msg2 = msg2 + ' | SPR: ' + str(round(session_profit_incfees_perc,2)) + "%" + '  $' + str(round(session_profit_incfees_total,2))
    msg2 = msg2 + ' | SPU: ' + str(round(unrealised_session_profit_incfees_perc,2)) + "%" + "  $" + str(round(unrealised_session_profit_incfees_total,2))
    msg2 = msg2 + ' | SPT: ' + str(round(session_profit_incfees_perc + unrealised_session_profit_incfees_perc,2)) + "%" + "  $" + str(round(session_profit_incfees_total+unrealised_session_profit_incfees_total,2))
    msg2 = msg2 + ' | ATP: ' + str(round(historic_profit_incfees_perc,2)) + "%" + "  $" + str(round(historic_profit_incfees_total,2))
    msg2 = msg2 + ' | TT: ' + str(trade_wins+trade_losses) + ' | TW: ' + str(trade_wins) + ' | TL: ' + str(trade_losses) + ' | WLR: ' + str(round(WIN_LOSS_PERCENT,2)) + "%" + " |"
...
def msg_discord_balance(msg1, msg2):
    ...
    if time_between_insertion.seconds > 60:
        if msg2 != discord_msg_balance_data:
            ...
            msg_discord('#' + str(datetime.now()).split('.')[0] + '#' +' |Horus Algorithmic|')
...
def pause_bot():
    ...
    while os.path.exists("signals/pausebot.pause"):
        ...
        if bot_paused == False:
            print(f'{txcolors.WARNING}Buying <PAUSED> SL and TP Working{txcolors.DEFAULT}')
            
            msg = '#' + str(datetime.now()).split('.')[0] + '#' + ' | PAUSEBOT: Buying <PAUSED> SL and TP Working'
            msg_discord(msg)
        ...
        if  bot_paused == True:
            print(f'{txcolors.WARNING}Resuming <BUYING> Total Sleep Time: {time_elapsed}{txcolors.DEFAULT}')
            
            msg = '#' + str(datetime.now()).split('.')[0] + '#' + ' | PAUSEBOT: Resuming <BUYING> Total Sleep Time: ' + str(time_elapsed)
            msg_discord(msg)
...
def convert_volume():
    ...
    for coin in volatile_coins:
        ...
        # calculate the volume in coin from TRADE_TOTAL in PAIR_WITH (default) #MODIFICADO 
        try:
        ...
        # define the volume with the correct step size #MODIFICADO
        if coin not in lot_size:
            ...
            if lot_size[coin] == 0:
                ...
                #volume[coin] = float('{:.{}f}'.format(volume[coin], lot_size[coin])) #MODIFICADO
                try:
...
def buy():
    ...
    for coin in volume:
        ...
        if coin not in coins_bought:
            print(f"{txcolors.BUY}Preparing to Buy [{volume[coin]}] <{coin}> @ ${last_price[coin]['price']}{txcolors.DEFAULT}")

            msg1 = '#' + str(datetime.now()).split('.')[0] + '#' + ' | BUY: <' + coin + '> | V: ' +  str(volume[coin]) + ' | P: @ ' + "$" + str(last_price[coin]['price']) + " |"
            msg_discord(msg1)
        ...
        # run the else block if the position has been placed and return order info
            ...
            print(f'{txcolors.WARNING}|Signal Detected| <IGNORED> Active Trade On: {coin}{txcolors.DEFAULT}')
...
def sell_coins(tpsl_override = False):
    ...
    for coin in list(coins_bought):
        ...
        if sellCoin:
            ...
            msg1 = '#' + str(datetime.now()).split('.')[0] + '#' + ' | SELL: <' + coin + '> | R: ' + sell_reason + ' | Result: ' + str(round(PriceChangeIncFees_Perc,2)) + "%" + ' @ ' + " $" + str(round(((float(coins_bought[coin]['volume'])*float(coins_bought[coin]['bought_at']))*PriceChangeIncFees_Perc)/100,4)) + " |"
            msg_discord(msg1)
    ...
    if len(coins_bought) == 0:
        if hsp_head == 1:
            print(f"|Horus Working| <NOT Coins Bought> ")
    else:
...
def check_total_session_profit(coins_bought, last_price):
    ...
    if DEBUG: 
        pass #print(f'|Horus Session Switches| |ASPP| <{allsession_profits_perc}%> |STP| <{SESSION_TAKE_PROFIT}%> |SSL| <{SESSION_STOP_LOSS}%>')
...
def update_portfolio(orders, last_price, volume):
    ...
    for coin in orders:
        ...
        if not TEST_MODE:
            ...
            print(f'{txcolors.BUY}Order For <{orders[coin]["symbol"]}> With ID [{orders[coin]["orderId"]}] Placed and Saved To File{txcolors.DEFAULT}')
        else:
            coins_bought[coin] = {
            ...
            print(f'{txcolors.BUY}Order For <{orders[coin][0]["symbol"]}> With ID [{orders[coin][0]["orderId"]}] Placed and Saved To File{txcolors.DEFAULT}')
...
def sell_all(msgreason, session_tspl_ovr = False):
    ...
    msgsell = '#' + str(datetime.now()).split('.')[0] + '#' + f' | SELL ALL COINS: {msgreason}'
    msg_discord(msgsell)
...
def menu():
    ...
    while LOOP:
        ...
        if x == 2:
            stop_signal_threads()
            start_signal_threads()
            print(f'{txcolors.WARNING}|HORUS|: {txcolors.WARNING}Modules Realoaded Completed{txcolors.DEFAULT}')
            LOOP = False
            ...
            print(f'\n\n\n')
            sellall = input(f'{txcolors.WARNING}|HORUS|: {txcolors.WARNING}Program Execution <ENDED> By User!\n\n ¿Do you want to sell all coins? (Y/N)?{txcolors.DEFAULT}')
            if sellall.upper() == "Y":
                # sell all coins
...
if __name__ == '__main__':
    ...
    if DEBUG:
        print(f'¡WELCOME!: Loaded Config Below\n{json.dumps(parsed_config, indent=4)}')
        print(f'{txcolors.BLUE}¡WELCOME!: Your Credentials Have Been Loaded From {creds_file}{txcolors.DEFAULT}')
    ...
    print(f'{txcolors.BLUE}°*°*°* Horus BY Cyb4rUf0 Systems *°*°*°')
    print(f'{txcolors.BLUE}|_ *SuperNatural* _|')
    print(f'{txcolors.BLUE}Press Ctrl-C To Stop The Script')
    ...
    while is_bot_running:
        try:
            ...
            if RESTART_EXTSIGNALS and thehour != datetime.now().hour:
                #stop_signal_threads() #modificacion
    ...
    if not is_bot_running:
        if SESSION_TPSL_OVERRIDE:
            ...
            print(f'')
            print(f'')
            print(f'{txcolors.WARNING}WARNING: ¡Horus Algorithmic Terminated For Some Reason!{txcolors.DEFAULT}')