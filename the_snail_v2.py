...
def do_work():
	while True:
		...
		print(f'{TextColors.TURQUOISE}|Horus The Snail| Checking For Potential Signals{TextColors.DEFAULT}')
		...
		if current_potential_list:
			...
			snail_discord = f'|Horus The Snail| Found [{snail_coins}] Coins and MACD Approved [{macd_coins}]'
			if DISCORD:
				msg_discord(snail_discord)
			print(f'{TextColors.TURQUOISE}|Horus The Snail| Found [{snail_coins}] Coins and MACD Approved [{macd_coins}]Coins |L: [{LIMIT}] Days |Min: {profit_min}% |Risk: {percent_below * 100}% | {TextColors.DEFAULT}')
		time.sleep(180)