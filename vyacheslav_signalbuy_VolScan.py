...
SCANNING_PERIOD = parsed_config['volscan_options']['SCANING_PERIOD']
...
TIME_SLEEP = parsed_config['volscan_options']['TIME_SLEEP']
...
CREATE_TICKER_LIST = parsed_config['volscan_options']['CREATE_TICKER_LIST']
...
NUMBER_COINS_IN_LIST = parsed_config['volscan_options']['NUMBER_COINS_IN_LIST']

# CV_INDEX - Coefficient of Variation. Only those coins with a COV greater than the specified value will be displayed.
CoV_INDEX = parsed_config['volscan_options']['CoV_INDEX']
...
CREATE_LIST_BY_COV_AND_PRICE_CHANGE = parsed_config['volscan_options']['CREATE_LIST_BY_COV_AND_PRICE_CHANGE']
...
CREATE_LIST_BY_ONLY_COV = parsed_config['volscan_options']['CREATE_LIST_BY_ONLY_COV']

#MODIFICACIONES
VOLSCAN_POSITIVE = parsed_config['volscan_options']['VOLSCAN_POSITIVE']
VOLSCAN_PRICE_THRESHOLD = parsed_config['volscan_options']['VOLSCAN_PRICE_THRESHOLD'] #Can be Negative or Positive

TOTAL_SCAN = round((SCANNING_PERIOD * 60) / TIME_SLEEP)
...
def do_work():
    ...
    while True:
        ...
        if VOLSCAN_POSITIVE:
            print(f'{txcolors.YELLOW}|Horus VolScan_UP| Launched With Scanning Period: [{SCANNING_PERIOD}] Minutes')
            print(f"|Horus VolScan_UP| Coins To Scan: [{len(init_price)}]")
        else:
            print(f'{txcolors.YELLOW}|Horus VolScan_DOWN| Launched With Scanning Period: [{SCANNING_PERIOD}] Minutes')
            print(f"|Horus VolScan_DOWN| Coins To Scan: [{len(init_price)}]")
        pass
        ...
        while count < (SCANNING_PERIOD * 60) / TIME_SLEEP:
            count += 1
            print(f'{txcolors.YELLOW}|Horus VolScan| Scanning Round [{count}/{TOTAL_SCAN}] Completed: Next Scan [{TIME_SLEEP}] Seconds{txcolors.DEFAULT}')
            try:
                    ...
                    if len(init_price[coin]['price_list']) == (SCANNING_PERIOD * 60) / TIME_SLEEP:
                        ...
                        if VOLSCAN_POSITIVE:
                            ...
                            if CREATE_LIST_BY_COV_AND_PRICE_CHANGE:
                                condition = percent_change_price > VOLSCAN_PRICE_THRESHOLD  and cov >= CoV_INDEX
                                
                            elif CREATE_LIST_BY_ONLY_COV:
                                condition = cov >= CoV_INDEX
                                
                            else:
                                condition = percent_change_price > VOLSCAN_PRICE_THRESHOLD
                ...
                if not list_volatility:
                    pass
                    #print(f'{txcolors.YELLOW}|Horus VolScan|{txcolors.DEFAULT}')
                else:
                    ...
                    for item in sort_list_vol_coin[:NUMBER_COINS_IN_LIST]:
                        print(f'{txcolors.YELLOW}|Horus VolScan| Detected Signal: {txcolors.END} '
